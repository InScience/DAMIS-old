...
void funcgrad(ap::real_1d_array x, double& f, ap::real_1d_array& g,
				double* pBasisNewD, double* pNewNewD, double* pBasisProj, int iBasisRows, int iSizeOfNew, int iProjDim) {
  double *pX, *pGr;
  int i, k;

  k = x.gethighbound() - x.getlowbound() + 1;

  pX = (double *) malloc (k * sizeof(double));
  pGr = (double *) calloc (iSizeOfNew * iProjDim, sizeof(double));

  for (i=x.getlowbound(); i<=x.gethighbound(); i++) {
    pX[i-1] = x(i);
    //pGr[i-1] = 0.0; nereikia kadangi naudojame calloc
  }

  Tools::RelativeMdsError(pBasisNewD, pNewNewD, pBasisProj, pX, iBasisRows, iSizeOfNew, iProjDim, &f, pGr);
	 ...
	 double* pBasisNewD, double* pNewNewD, double* pBasisProj, int iBasisRows, int iSizeOfNew, int iProjDim)
{
    ...
    g.setbounds(1, n * iSizeOfNew);
    xold.setbounds(1, n);
    tx.setbounds(1, n);
    diag.setbounds(1, n);
    ta.setbounds(1, n);
    funcgrad(x, f, g, pBasisNewD, pNewNewD, pBasisProj, iBasisRows, iSizeOfNew, iProjDim);
    fold = f;
    iter = 0;
    info = 0;
    ...
    while(true)
    {
        ...
        lbfgsmcsrch(n, x, f, g, w, ispt+point*n+1, stp, ftol, xtol, maxfev, info, nfev, diag, gtol, stpmin, stpmax, pBasisNewD, pNewNewD, pBasisProj, iBasisRows, iSizeOfNew, iProjDim);
        if( info!=1 )
        {
            if( info==0 )
            {
